...
module Network.Gitit.Authentication.Github (loginUserOAuth, getGithubOAuthToken, GithubCallbackPars) where
...
loginUserOAuth :: OAuth2 -> Handler
loginUserOAuth githubKey = do
...
getGithubOAuthToken :: OAuth2 
                    -> GithubCallbackPars                  -- ^ Authentication code gained after authorization
                    -> IO (Either BSL.ByteString User)
getGithubOAuthToken githubKey githubCallbackPars = do
  ...
  let mUser = case token of
                ...
                Left err ->
                     return $ Left $  "no access token found yet: " `BSL.append` err
  closeManager mgr
  mUser
